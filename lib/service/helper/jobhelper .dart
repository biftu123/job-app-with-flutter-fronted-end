...
class JobHelper {
  static Future<List<JobResponse>> getJobs() async {
    ...
    final response = await client.get(
      Uri.parse(
          "http://192.168.100.4:3001/job"), // Replace with your API endpoint
  ...
  static Future<Getjobres> getJob(String jobId) async {
    ...
    try {
      final response = await client.get(
        Uri.parse('http://192.168.100.4:3001/job/${jobId}'),
        headers: requestHeaders,
      );
  ...
  static Future<List<JobResponse>> searchjob(String searchkey) async {
    ...
    final response = await client.get(
      Uri.parse(
          "http://192.168.100.4:3001/job/search/${searchkey}"), // Replace with your API endpoint
      headers: requestHeaders,
    );
